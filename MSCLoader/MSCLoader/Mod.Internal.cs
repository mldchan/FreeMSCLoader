...
public partial class Mod
{
    ...
    internal List<Keybind> DefaultKeybinds = new();
    internal bool disableWarn = false;
    internal string fileName;
    internal bool hasUpdate = false;
    internal bool hideResetAllSettings = false;
    internal List<Keybind> Keybinds = new();
    internal bool menuCallbacks;
    internal MSCLData metadata; //Local metadata
    internal int modErrors = 0;

    internal List<ModSetting> modSettingsList = new();
    internal bool newEnDisFormat;
    internal bool newFormat;
    internal bool newSettingsFormat;
        ...
        switch (functionType)
        {
            case Setup.OnNewGame:
                if (A_OnNewGame == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnNewGame</b> function type.");
                break;
            case Setup.OnMenuLoad:
                ...
                if (A_OnMenuLoad == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnMenuLoad</b> function type.");
                break;
            case Setup.PreLoad:
                if (A_PreLoad == null)
                    A_PreLoad = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>PreLoad</b> function type.");
                break;
            case Setup.OnLoad:
                if (A_OnLoad == null)
                    A_OnLoad = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnLoad</b> function type.");
                break;
            case Setup.PostLoad:
                if (A_PostLoad == null)
                    A_PostLoad = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>PostLoad</b> function type.");
                break;
            case Setup.OnSave:
                if (A_OnSave == null)
                    A_OnSave = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnSave</b> function type.");
                break;
            case Setup.OnGUI:
                if (A_OnGUI == null)
                    A_OnGUI = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnGUI</b> function type.");
                break;
            case Setup.Update:
                if (A_Update == null)
                    A_Update = function;
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>Update</b> function type.");
                break;
            case Setup.FixedUpdate:
                if (A_FixedUpdate == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>FixedUpdate</b> function type.");
                break;
            case Setup.OnModEnabled:
                ...
                if (A_OnModEnabled == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnModEnabled</b> function type.");
                break;
            case Setup.OnModDisabled:
                ...
                if (A_OnModDisabled == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>OnModDisabled</b> function type.");
                break;
            case Setup.ModSettingsLoaded:
                ...
                if (A_ModSettingsLoaded == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>ModSettingsLoaded</b> function type.");
                break;
            case Setup.ModSettings:
                ...
                if (A_ModSettings == null)
                    ...
                    ModConsole.Error(
                        $"SetupMod() error for <b>{ID}</b>. You already created <b>ModSettings</b> function type.");
                break;
            default:
