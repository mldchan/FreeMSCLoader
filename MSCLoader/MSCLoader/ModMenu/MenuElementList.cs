...
{
    [Header("Values")] public RawImage icon;
    ...
    [Header("Additional Values")] public Button DownloadUpdateBtn, OpenDownloadWebsiteBtn;

    public Text DownloadInfoTxt;

    [Header("Anim stuff")] public GameObject quickInfo, warningInfo;
    ...
    private IEnumerator Anim(bool expand)
    {
        anim = true;
        if (expand)
        {
            for (var i = 0; i < 50; i++)
            {
                contents.localScale = new Vector3(1, (float)Math.Round(contents.localScale.y + 0.02f, 2), 1);
                yield return null;
            }
    ...
    public void ModInfoFill()
    {
        if (mod != null)
        {
            ...
            if (mod.metadata != null)
            {
                ...
                {
                    if (File.Exists(
                            Path.Combine(ModLoader.MetadataFolder, Path.Combine("Mod Icons", mod.metadata.icon))))
                        try
                        {
                            var t2d = new Texture2D(1, 1);
                            t2d.LoadImage(File.ReadAllBytes(Path.Combine(ModLoader.MetadataFolder,
                                Path.Combine("Mod Icons", mod.metadata.icon))));
                            icon.texture = t2d;
                        }
                        catch (Exception e)
                        {
    ...
    public void ModButtonsPrep(UniversalView uv)
    {
        if (Settings.Get(mod).Count > 0)
        {
            ...
            SettingsBtn.onClick.AddListener(delegate { uv.FillSettings(mod); });
        }

        if (Keybind.Get(mod).Count > 0)
        {
            KeybindsBtn.gameObject.SetActive(true);
            KeybindsBtn.onClick.AddListener(delegate { uv.FillKeybinds(mod); });
        }

        MoreInfoBtn.onClick.AddListener(delegate { uv.FillMetadataInfo(mod); });
    }
    ...
    public void InvalidMod(InvalidMods mod)
    {
        ...
        Description.text =
            $"Failed to load this mod{Environment.NewLine}Error: <color=yellow>{mod.ErrorMessage}</color>";
        WarningInfo.text =
            $"<color=orange>This Mod failed to load.</color>{Environment.NewLine}Error: <color=yellow>{mod.ErrorMessage}</color>";
        if (mod.IsManaged)
        {
            QuickInfo.text =
                $"<color=orange>This mod failed to load.</color>{Environment.NewLine}Guid: <color=yellow>{mod.AsmGuid}</color>";
            if (mod.AdditionalRefs.Count > 0)
                QuickInfo.text +=
                    $"{Environment.NewLine}Additional references: <color=aqua>{string.Join(", ", mod.AdditionalRefs.ToArray())}</color>";
        }
        else
        {
    ...
    public void ReferenceInfoFill(References rf)
    {
        ...
        if (ModLoader.Instance.ReferencesList.Where(x => x.Guid == rf.Guid).ToArray().Length > 1)
        {
            ...
            WarningBtn.gameObject.SetActive(true);
            WarningInfo.text =
                $"<color=yellow>This Reference has been detected to be loaded more than once.</color>{Environment.NewLine}";
            WarningInfo.text += $"This can cause namespace conflict issues.{Environment.NewLine}";
            WarningInfo.text +=
                $"<color=yellow>If you created this, make sure to keep same file name when releasing new version of reference.</color>{Environment.NewLine}";
        }
        ...
        if (string.IsNullOrEmpty(rf.AssemblyAuthor))
            ...
            Author.text =
                $"by <color=orange>{rf.AssemblyAuthor}</color> (<color=aqua>{rf.AssemblyFileVersion}</color>)";
        ...
        if (string.IsNullOrEmpty(rf.AssemblyAuthor))
            ...
            QuickInfo.text +=
                $"<color=yellow>Author:</color> <color=aqua>{rf.AssemblyAuthor}</color>{Environment.NewLine}";
        QuickInfo.text +=
            $"<color=yellow>Version:</color> <color=aqua>{rf.AssemblyFileVersion}</color>{Environment.NewLine}";
    ...
    public void UpdateInfoFill()
    {
        if (refs != null)
        {
            ...
            if (string.IsNullOrEmpty(refs.AssemblyAuthor))
                ...
                Author.text =
                    $"by <color=orange>{refs.AssemblyAuthor}</color> (<color=aqua>{refs.AssemblyFileVersion}</color>)";
            ...
            MoreInfoBtn.onClick.AddListener(() =>
                ShowChangelog(refs.AssemblyID, refs.UpdateInfo.ref_version, refs.AssemblyTitle));
    ...
    public void DisableThisMod(bool ischecked)
    {
        if (mod.isDisabled != ischecked)
        {
            if (mod.metadata != null)
                ...
                {
                    ModUI.ShowMessage(
                        $"This mod cannot be enabled. {Environment.NewLine}Reason: <color=yellow>{mod.metadata.msg}</color>");
                    return;
                }

            var onEnableExist = false;
            ...
            if (ischecked)
            {
                ...
                {
                    ModUI.ShowMessage(
                        "This mod was loaded in Main Menu, to fully disable this mod you need to restart the game.");
                    ModConsole.Print(
                        $"Mod <b><color=orange>{mod.Name}</color></b> marked to be disabled <color=red><b>Disabled</b></color> on next restart");
                }
                else
                {
                    ModConsole.Print(
                        $"Mod <b><color=orange>{mod.Name}</color></b> has been <color=red><b>Disabled</b></color>");
                }
            }
            else
            {
                ...
                if (onEnableExist)
                    ModConsole.Print(
                        $"Mod <b><color=orange>{mod.Name}</color></b> has been <color=green><b>Enabled</b></color>");
                else
                    ModConsole.Print(
                        $"Mod <b><color=orange>{mod.Name}</color></b> has been <color=green><b>Enabled</b></color>, restart may be required.");
            }
