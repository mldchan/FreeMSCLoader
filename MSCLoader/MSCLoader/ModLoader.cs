...
public partial class ModLoader : MonoBehaviour
{
    ...
    internal static string GetMetadataFolder(string fn)
    {
        return Path.Combine(MetadataFolder, fn);
    }
    ...
    {
        if (unloader) return;
        ModsFolder = Path.GetFullPath(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
    ...
    private static void PrepareModLoader()
    {
        ...
        {
            loaderPrepared = true;
            var go = new GameObject("MSCLoader", typeof(ModLoader));
    ...
    private void OnLevelWasLoaded(int level)
    {
        switch (Application.loadedLevelName)
        {
            case "MainMenu":
                ...
                if (GameObject.Find("MSCLoader Info") == null) MainMenuInfo();
                if (allModsLoaded)
                {
    ...
    private void Init()
    {
        ...
        if (!Directory.Exists(SettingsFolder) && Directory.Exists(ConfigFolder))
        {
            ...
            foreach (var dir in Directory.GetDirectories(ConfigFolder))
        ...
        {
            var go = new GameObject("MSCUnloader", typeof(MSCUnloader));
    ...
    private void ContinueInit()
    {
        ...
        if (InvalidMods.Count > 0)
            ModConsole.Print(
                $"<b><color=orange>Loaded <color=aqua>{actualModList.Length}</color> mods (<color=magenta>{InvalidMods.Count}</color> failed to load)!</color></b>{Environment.NewLine}");
        else
            ModConsole.Print(
                $"<b><color=orange>Loaded <color=aqua>{actualModList.Length}</color> mods!</color></b>{Environment.NewLine}");
    ...
    private void HandleCCanv(GameObject go)
    {
        StartCoroutine(HandleCanvC(go));
    }

    private IEnumerator HandleCanvC(GameObject go)
    {
        yield return null;
        go.SetActive(true);
    }
    ...
    internal void CheckForModsUpd(bool force = false)
    {
        ...
        else
        {
            ...
            {
                var s = File.ReadAllText(Path.Combine(SettingsFolder,
                    Path.Combine("MSCLoader_Settings", "updateInfo.json")));
                var v = JsonConvert.DeserializeObject<ModVersions>(s);
                ModMetadata.ReadUpdateInfo(v);
                if (File.Exists(Path.Combine(SettingsFolder,
                        Path.Combine("MSCLoader_Settings", "ref_updateInfo.json"))))
                {
                    var s2 = File.ReadAllText(Path.Combine(SettingsFolder,
                        Path.Combine("MSCLoader_Settings", "ref_updateInfo.json")));
                    var v2 = JsonConvert.DeserializeObject<RefVersions>(s2);
    ...
    private void SAuthCheckCompleted(object sender, DownloadStringCompletedEventArgs e)
    {
        try
        {
            ...
            if (result != string.Empty)
            {
                ...
                if (ed[0] == "ok")
                {
                    ...
                    var sp = Path.Combine(SettingsFolder, Path.Combine("MSCLoader_Settings", "otk.bin"));
                    var st = new FileStream(sp, FileMode.Create);
                    f.Serialize(st, s);
                    st.Close();
                }
                else
            ...
            if (ret && ModMenu.expWarning.GetValue())
                if (!Name.StartsWith("default_")) //default is NOT experimental branch
                    ModUI.ShowMessage(
                        $"<color=orange><b>Warning:</b></color>{Environment.NewLine}You are using beta build: <color=orange><b>{Name}</b></color>{Environment.NewLine}{Environment.NewLine}Remember that some mods may not work correctly on beta branches.",
                        "Experimental build warning");
            Console.WriteLine($"MSC buildID: <b>{SteamApps.GetAppBuildId()}</b>");
        ...
        catch (Exception ex)
        {
            ...
            if (e.Error != null)
            {
                ...
                {
                    var f = new BinaryFormatter();
                    var st = new FileStream(sp, FileMode.Open);
                    var s = f.Deserialize(st) as SaveOtk;
                    st.Close();
                    var otk = "otk_" + SidChecksumCalculator($"{steamID}{s.k1}");
    ...
    private void LoadReferences()
    {
        ...
        {
            var files = Directory.GetFiles(Path.Combine(ModsFolder, "References"), "*.dll");
            var managedStuff = Directory.GetFiles(Path.Combine("mysummercar_Data", "Managed"), "*.dll");
            var alreadyIncluded = (from s in managedStuff select Path.GetFileName(s)).ToArray();
            for (var i = 0; i < files.Length; i++)
            {
                if (Path.GetFileName(files[i]) == "0Harmony12.dll" ||
                    Path.GetFileName(files[i]) == "0Harmony-1.2.dll" ||
                    alreadyIncluded.Contains(Path.GetFileName(files[i])))
                {
                    ModConsole.Warning(
                        $"<b>{Path.GetFileName(files[i])}</b> already exist in <b>{Path.GetFullPath(Path.Combine("mysummercar_Data", "Managed"))}</b> - skipping");
                    File.Delete(files[i]);
                    continue;
                }
                ...
                try
                {
                    var asm = Assembly.LoadFrom(files[i]);
    ...
    private void LoadReferencesMeta(Assembly ass, string fn)
    {
        ...
        if (Attribute.IsDefined(ass, typeof(AssemblyTitleAttribute)))
            reference.AssemblyTitle =
                ((AssemblyTitleAttribute)Attribute.GetCustomAttribute(ass, typeof(AssemblyTitleAttribute))).Title;
        if (Attribute.IsDefined(ass, typeof(AssemblyCompanyAttribute)))
            reference.AssemblyAuthor =
        ...
        if (Attribute.IsDefined(ass, typeof(AssemblyDescriptionAttribute)))
            reference.AssemblyDescription =
                ((AssemblyDescriptionAttribute)Attribute.GetCustomAttribute(ass, typeof(AssemblyDescriptionAttribute)))
                .Description;
        if (Attribute.IsDefined(ass, typeof(AssemblyFileVersionAttribute)))
            reference.AssemblyFileVersion =
                ((AssemblyFileVersionAttribute)Attribute.GetCustomAttribute(ass, typeof(AssemblyFileVersionAttribute)))
                .Version;
        if (Attribute.IsDefined(ass, typeof(GuidAttribute)))
            reference.Guid = ((GuidAttribute)Attribute.GetCustomAttribute(ass, typeof(GuidAttribute))).Value;
        ReferencesList.Add(reference);
    }
    ...
    private void LoadCoreAssets()
    {
        ...
        var ab = LoadAssets.LoadBundle("MSCLoader.CoreAssets.core.unity3d");
        ...
        var loadingP = ab.LoadAsset<GameObject>("MSCLoader Canvas loading.prefab");
        var mbP = ab.LoadAsset<GameObject>("MSCLoader Canvas msgbox.prefab");

        ModUI.PrepareDefaultCanvas();
        var mb = Instantiate(mbP);
    ...
    {
        var errorDetails =
            $"{Environment.NewLine}<b>Details: </b>{e.Message} in <b>{new StackTrace(e, true).GetFrame(0).GetMethod()}</b>";
        if (save)
    ...
    private IEnumerator NewGameMods()
    {
        ...
        for (var i = 0; i < Mod_OnNewGame.Length; i++)
        {
        ...
        for (var i = 0; i < BC_ModList.Length; i++)
        {
    ...
    private void MouseFix()
    {
        var curFSM = GameObject.Find("PLAYER").GetPlayMaker("Update Cursor");
        var curLock = curFSM.GetState("Update cursor");
        var curUnLock = curFSM.GetState("In Menu");
    ...
    private IEnumerator LoadMods()
    {
        ...
        var totalCount = PLoadMods.Length + BC_ModList.Length + SecondPassMods.Length + Mod_PreLoad.Length +
                         Mod_OnLoad.Length + Mod_PostLoad.Length;
        canvLoading.SetLoading("Loading mods - Phase 1", 0, totalCount, "Loading mods. Please wait...");
        yield return null;
        for (var i = 0; i < Mod_PreLoad.Length; i++)
        {
        ...
        for (var i = 0; i < PLoadMods.Length; i++)
        {
        ...
        yield return null;
        for (var i = 0; i < Mod_OnLoad.Length; i++)
        {
        ...
        yield return null;
        for (var i = 0; i < Mod_PostLoad.Length; i++)
        {
        ...
        for (var i = 0; i < SecondPassMods.Length; i++)
        {
    ...
    private static bool wasSaving;

    private void SaveMods()
    {
        saveErrors = new List<string>();
        wasSaving = true;
        for (var i = 0; i < Mod_OnSave.Length; i++)
        {
        ...
        for (var i = 0; i < OnSaveMods.Length; i++)
        {
    ...
    private void PreLoadMods()
    {
        ...
        SecondPassMods = BC_ModList
            .Where(x => CheckEmptyMethod(x, "PostLoad") || CheckEmptyMethod(x, "SecondPassOnLoad")).ToArray();
        ...
        if (!devMode)
        {
            var cleanupLast = Path.Combine(SettingsFolder, Path.Combine("MSCLoader_Settings", "lastCleanupCheck"));
            if (File.Exists(cleanupLast))
            {
                var lastCheckS = File.ReadAllText(cleanupLast);
                DateTime.TryParse(lastCheckS, out var lastCheck);
                if ((DateTime.Now - lastCheck).TotalDays >= 14 || (DateTime.Now - lastCheck).TotalDays < 0)
                {
                    ...
                    if (found)
                        ModUI.ShowYesNoMessage(
                            $"There are unused mod files/assets that can be cleaned up.{Environment.NewLine}{Environment.NewLine}List of unused mod files:{Environment.NewLine}<color=aqua>{string.Join(", ", cleanupList.ToArray())}</color>{Environment.NewLine}Do you want to clean them up?",
                            "Unused files found", CleanupFolders);
    ...
    private void GetRequiredFilesData()
    {
        //  modIDsReferences = new List<string>();
        if (modIDsReferences.Count == 0 && crashedGuids.Count == 0)
            return;
        var missingModIDsReferences = new List<string>();
        for (var i = 0; i < modIDsReferences.Count; i++)
    ...
    {
        var setFold = Directory.GetDirectories(SettingsFolder);
        for (var i = 0; i < setFold.Length; i++)
        ...
        var assFold = Directory.GetDirectories(AssetsFolder);
        for (var i = 0; i < assFold.Length; i++)
    ...
    private void LoadModsSettings()
    {
        for (var i = 0; i < LoadedMods.Count; i++)
        {
            ...
            try
            {
                ...
                {
                    if (LoadedMods[i].A_ModSettings != null) LoadedMods[i].A_ModSettings.Invoke();
                }
                else
                {
            ...
            {
                if (LoadedMods[i].proSettings)
                {
                    Console.WriteLine(e); // No need to spam console with pro settings errors.
                }
                else
                {
                    ModConsole.Error(
                        $"Settings error for mod <b>{LoadedMods[i].ID}</b>{Environment.NewLine}<b>Details:</b> {e.Message}");
                    if (devMode)
                        ModConsole.Error(e.ToString());
                    Console.WriteLine(e);
                }
    ...
    private void LoadDLL(string file, byte[] byteFile = null)
    {
        ...
        try
        {
            ...
            var isMod = false;
            var list = asm.GetReferencedAssemblies();
            if (Attribute.IsDefined(asm, typeof(GuidAttribute)))
                asmGuid = ((GuidAttribute)Attribute.GetCustomAttribute(asm, typeof(GuidAttribute))).Value;
            Console.WriteLine($"GUID: {asmGuid}");
            string msVer = null;
            for (var i = 0; i < list.Length; i++)
            {
                if (!stdRef.Contains(list[i].Name)) addRef.Add(list[i].Name);
                if (list[i].Name == "MSCLoader")
                {
                    mscl = true;
                    var verparse = list[i].Version.ToString().Split('.');
            ...
            if (!asm.ImageRuntimeVersion.Equals(Assembly.GetExecutingAssembly().ImageRuntimeVersion))
                ModConsole.Warning(
                    $"File <b>{Path.GetFileName(file)}</b> is targeting runtime version <b>{asm.ImageRuntimeVersion}</b> which is different than current running version <b>{Assembly.GetExecutingAssembly().ImageRuntimeVersion}</b>. This may cause unexpected behaviours, check your target assembly.");

            // Look through all public classes
            var asmTypes = asm.GetTypes();

            for (var j = 0; j < asmTypes.Length; j++)
            {
                ...
                {
                    var m = (Mod)Activator.CreateInstance(asmTypes[j]);
            ...
            if (!isMod)
            {
                ...
                ModConsole.Error(
                    $"<b>{Path.GetFileName(file)}</b> - doesn't look like a mod or missing Mod subclass!{Environment.NewLine}<b>Details:</b> File loaded correctly, but failed to find Mod methods.{Environment.NewLine}If this is a reference put this file into \"<b>References</b>\" folder.{Environment.NewLine}");
                InvalidMods.Add(new InvalidMods(Path.GetFileName(file), true,
                    "File loaded correctly, but failed to find Mod methods.", addRef.ToList(), asmGuid));
            }
        }
        catch (Exception e)
        {
            if (mscl)
            {
                crashedGuids.Add(asmGuid);
                ModConsole.Error(
                    $"<b>{Path.GetFileName(file)}</b> - crashed during load.<b>Details:</b> {e.GetFullMessage()}");
                InvalidMods.Add(new InvalidMods(Path.GetFileName(file), true, e.GetFullMessage(), addRef.ToList(),
                    asmGuid));
                if (addRef.Count > 0)
                    ModConsole.Print(
            ...
            else
            {
                ...
                {
                    ModConsole.Error(
                        $"<b>{Path.GetFileName(file)}</b> - failed to load this as valid early access file. Most likely there is new updated file available!{Environment.NewLine}<b>Details:</b> {e.GetType().Name}{Environment.NewLine}");
                    InvalidMods.Add(new InvalidMods(Path.GetFileName(file), false,
                        "failed to load this as valid early access file. Most likely there is new updated file available!"));
                }
                else
                {
                    if (string.IsNullOrEmpty(e.Message))
                        ModConsole.Error(
                            $"<b>{Path.GetFileName(file)}</b> - doesn't look like a mod, remove this file from mods folder!{Environment.NewLine}<b>Details:</b> {e.GetType().Name}{Environment.NewLine}");
                    else
                        ModConsole.Error(
                            $"<b>{Path.GetFileName(file)}</b> - doesn't look like a mod, remove this file from mods folder!{Environment.NewLine}<b>Details:</b> {e.GetFullMessage()}{Environment.NewLine}");
    ...
    private bool ReferencesArePresent(string[] refs)
    {
        ...
        for (var i = 0; i < LoadedAsms.Length; i++)
    ...
    private void LoadMod(Mod mod, string msver, string fname = null, string[] additionalRef = null)
    {
        ...
        if (!LoadedMods.Contains(mod) && !LoadedMods.Select(x => x.ID).Contains(mod.ID))
        {
            ...
            if (mod.AdditionalReferences != null) modIDsReferences.Add(mod.ID);
            try
            {
                mod.ModSetup();
                if (mod.newFormat && mod.fileName == null) mod.A_OnMenuLoad?.Invoke();
            }
            catch (Exception e)
            {
                ModException(e, mod);
    ...
    {
        GUI.skin = guiskin;
        for (var i = 0; i < Mod_OnGUI.Length; i++)
        {
    ...
    {
        GUI.skin = guiskin;
        for (var i = 0; i < OnGUImods.Length; i++)
        {
    ...
    {
        for (var i = 0; i < Mod_Update.Length; i++)
        {
    ...
    {
        for (var i = 0; i < UpdateMods.Length; i++)
        {
    ...
    {
        for (var i = 0; i < Mod_FixedUpdate.Length; i++)
        {
    ...
    {
        for (var i = 0; i < FixedUpdateMods.Length; i++)
        {
    ...
    private void ModExceptionHandler(Exception e, Mod mod)
    {
        if (LogAllErrors) ModException(e, mod);
        if (allModsLoaded)
            mod.modErrors++;
        if (devMode)
        {
        ...
        else
        {
            if (mod.modErrors >= 30)
            {
                mod.isDisabled = true;
                ModConsole.Error(
                    $"Mod <b>{mod.ID}</b> has been <b>disabled!</b> Because it spams too many errors each frame!{Environment.NewLine}Report this problem to mod author.{Environment.NewLine}Last error message:");
    ...
    {
        var Sinfo = SystemInfo.operatingSystem;
        try
        {
            if (Sinfo.Contains("Windows"))
            {
                var windowsfixed = Sinfo;
                var build = int.Parse(Sinfo.Split('(')[1].Split(')')[0].Split('.')[2]);
                if (build > 21999)
                {
