...
internal class Manager : MonoBehaviour
{
    ...
    private static readonly Queue<AudioInstance> deferredLoadQueue;

    private static readonly Queue<AudioInstance> deferredSetDataQueue;

    private static readonly Queue<AudioInstance> deferredSetFail;

    private static Thread deferredLoaderThread;

    private static GameObject managerInstance;

    private static readonly Dictionary<AudioClip, AudioClipLoadType> audioClipLoadType;

    private static readonly Dictionary<AudioClip, AudioDataLoadState> audioLoadState;
    ...
    public static AudioClip Load(string filePath, bool doStream = false, bool loadInBackground = true,
        bool useCache = true)
    {
        if (!IsSupportedFormat(filePath))
        {
            Console.WriteLine(
                $"Could not load AudioClip at path '{filePath}' it's extensions marks unsupported format, supported formats are: {string.Join(", ", Enum.GetNames(typeof(AudioFormat)))}");
            return null;
        }

        AudioClip audioClip;
        if (useCache && cache.TryGetValue(filePath, out audioClip) && audioClip) return audioClip;
        var streamReader = new StreamReader(filePath);
        audioClip = Load(streamReader.BaseStream, GetAudioFormat(filePath), filePath, doStream, loadInBackground);
        if (useCache) cache[filePath] = audioClip;
        return audioClip;
    }

    public static AudioClip Load(Stream dataStream, AudioFormat audioFormat, string unityAudioClipName,
        bool doStream = false, bool loadInBackground = true, bool diposeDataStreamIfNotNeeded = true)
    {
        ...
        try
        {
            ...
            if (doStream)
            {
                audioClip = AudioClip.Create(unityAudioClipName, audioInstance.samplesCount / audioInstance.channels,
                    audioInstance.channels, audioInstance.sampleRate, doStream,
                    delegate(float[] target) { reader.Read(target, 0, target.Length); }, delegate(int target)
                    {
        ...
        catch (Exception ex)
        {
            ...
            Console.WriteLine(
                string.Concat("Could not load AudioClip named '", unityAudioClipName, "', exception:", ex));
        }
    ...
    {
        var result = AudioDataLoadState.Failed;
        if (audioClip != null)
        {
    ...
    {
        var result = (AudioClipLoadType)(-1);
        if (audioClip != null)
        {
    ...
    {
        var result = AudioFormat.unknown;
        try
        {
