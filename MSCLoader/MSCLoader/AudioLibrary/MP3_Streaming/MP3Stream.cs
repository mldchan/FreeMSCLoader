...
internal class MP3Stream : IDisposable
{
    ...
    {
        StopPlayback();
        Console.WriteLine("[MP3Stream] Disposed");
    }
    ...
    private void StreamMp3(object state)
    {
        ...
        var metaInt = 0; // blocksize of mp3 data
        ...
        {
            if (e.Status != WebExceptionStatus.RequestCanceled) Console.WriteLine(e.Message);
            return;
        }
    ...
    {
        if (bufferedWaveProvider != null) bufferedWaveProvider.ToSampleProvider().Read(data, 0, data.Length);
    }
    ...
    private void StopPlayback()
    {
        if (playbackState != StreamingPlaybackState.Stopped)
        {
            if (!fullyDownloaded) webRequest.Abort();
    ...
    private void OnPlaybackStopped(object sender, StoppedEventArgs e)
    {
        ...
        if (e.Exception != null) Console.WriteLine("Playback Error {0}", e.Exception.Message);
    }
