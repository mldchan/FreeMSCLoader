...
public static class MSCExtensions
{
    ...
    public static void MakePickable(this Transform transform)
    {
        LoadAssets.MakeGameObjectPickable(transform.gameObject);
    }
}
...
public static class PlayMakerExtensions
{
    ...
    {
        var pm = obj.GetComponents<PlayMakerFSM>();
    ...
    public static FsmState GetPlayMakerState(this GameObject obj, string stateName)
    {
        ...
        {
            for (var i = 0; i < pm.Length; i++)
            for (var j = 0; j < pm[i].FsmStates.Length; j++)
    ...
    {
        var pm = gameObject.GetComponents<PlayMakerFSM>();
        if (pm != null && pm.Length > 0)
            for (var i = 0; i < pm.Length; i++)
            for (var j = 0; j < pm[i].FsmStates.Length; j++)
                if (pm[i].FsmStates[j].Name == stateName)
    ...
    public static bool FsmInject(this GameObject go, string playmakerName, string stateName, Action hook,
        bool everyFrame = false, int index = -1, bool replace = false)
    {
    ...
    /// <param name="replace">If true, replaces the action at specified index, otherwise inserts it.</param>
    /// <returns>Whether the injection was successful</returns>
    public static bool FsmInject(this Transform tf, string playmakerName, string stateName, Action hook,
        bool everyFrame = false, int index = -1, bool replace = false)
    {
    ...
    /// <param name="replace">If true, replaces the action at specified index, otherwise inserts it.</param>
    /// <returns>Whether the injection was successful</returns>
    public static bool FsmInject(this PlayMakerFSM fsm, string stateName, Action hook, bool everyFrame = false,
        int index = -1, bool replace = false)
    {
    ...
    /// <param name="replace">If true, replaces the action at specified index, otherwise inserts it.</param>
    /// <returns>Whether the injection was successful</returns>
    public static bool FsmInject<T>(this PlayMakerFSM fsm, string stateName, T action, int index = -1,
        bool replace = false) where T : FsmStateAction
    {
        ...
        else
        {
            ...
            if (index == -1) _a[_a.Length - 1] = action;
    ...
    {
        for (var i = 0; i < fsm.FsmEvents.Length; i++)
    ...
    {
        pm.InitializeFSM();
        for (var i = 0; i < pm.FsmGlobalTransitions.Length; i++)
    ...
    public static T GetVariable<T>(this PlayMakerFSM pm, string ID) where T : new()
    {
        try
        {
            ...
            switch (typeof(T))
            {
                case Type t when t == typeof(FsmFloat):
                    var = pm.Fsm.Variables.FloatVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmInt):
                    var = pm.Fsm.Variables.IntVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmBool):
                    var = pm.Fsm.Variables.BoolVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmGameObject):
                    var = pm.Fsm.Variables.GameObjectVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmString):
                    var = pm.Fsm.Variables.StringVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmVector2):
                    var = pm.Fsm.Variables.Vector2Variables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmVector3):
                    var = pm.Fsm.Variables.Vector3Variables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmColor):
                    var = pm.Fsm.Variables.ColorVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmRect):
                    var = pm.Fsm.Variables.RectVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmMaterial):
                    var = pm.Fsm.Variables.MaterialVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmTexture):
                    var = pm.Fsm.Variables.TextureVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmQuaternion):
                    var = pm.Fsm.Variables.QuaternionVariables.First(x => x.Name == ID); break;
                case Type t when t == typeof(FsmObject):
                    var = pm.Fsm.Variables.ObjectVariables.First(x => x.Name == ID); break;
            }
    ...
    {
        pm.InitializeFSM();
        for (var i = 0; i < pm.FsmStates.Length; i++)
    ...
    {
        for (var i = 0; i < fs.Transitions.Length; i++)
    ...
    public static void AddActions(this FsmState fs, ICollection<FsmStateAction> actions)
    {
    ...
    public static void InsertAction(this FsmState fs, int index, FsmStateAction action)
    {
        ...
        if (index < 0 || index >= fs.Actions.Length) return;
    ...
    private class MSCL_PMHook : FsmStateAction
    {
        ...
        public readonly bool everyFrame;
